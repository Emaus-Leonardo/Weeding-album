...
import ImageSkelet from "../layout/ImageSkelet";
    ...
    <div className="p-5 px-40 tablet:px-10 md:px-10 md:p-19 bg-custom transition-all">
      ...
      >
        {fotos.map((foto, index) => (
          <div className="py-4" key={index} onClick={() => openFullScreen(index)}>
            <ImageSkelet src={foto} alt={`Foto ${index + 1}`} />
          </div>
        ))}
      </div>
      ...
      {selectedImageIndex !== null && (
        <div className="fixed top-0 left-0 z-50 w-full h-full flex items-center justify-center bg-black bg-opacity-80">
          ...
          />
          <button
            className="absolute top-1/2 left-4 transform -translate-y-1/2 bg-white text-gray-800 px-2 py-2 hover:scale-110 transition-all"
            onClick={previousImage}
          >
            <FiChevronLeft />
          </button>
          <button
            className="absolute top-1/2 right-4 transform -translate-y-1/2 bg-white text-gray-800 px-2 py-2 hover:scale-110 transition-all"
            onClick={nextImage}
          >
            <FiChevronRight />
          </button>
          <button
            className="absolute top-4 right-4 bg-white text-gray-800 px-2 py-1.5 hover:scale-110 transition-all"
